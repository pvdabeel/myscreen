#!/usr/bin/env PYTHONIOENCODING=UTF-8 PATH=/usr/local/bin:/opt/local/bin:/opt/homebrew/bin:/usr/bin:$PATH python3
# -*- coding: utf-8 -*-
#
...
def list_screens():
    """Yield (name, status) tuples for current `screen` sessions."""
    out = subprocess.run(
        ['screen', '-ls'], capture_output=True, text=True
    ).stdout
    for match in re.finditer(r'^\s*\d+\.(\S+)\s+\(([^)]+)\)', out, re.M):
